...
# Inicializar estados de sesión si es necesario
...
def calcular_modelo_manheim(T, A, capacidad_total):
    """
    Calcula la tasa de saturación y nivel de servicio basándose en la oferta (T),
    la demanda ajustada (A) y la capacidad total del sistema.
    """
    ...
    # Nivel de servicio en escala inversa a la saturación (ej. 1 a 5 o similar)
    if saturacion <= 70:
        nivel_servicio = 5.0
...
with tab1:
    ...
    with col_c1:
        num_trenes = st.slider("🚆 Trenes (L6/L7)", 10, 50, 25, key="trenes_cetram_a_unico")
    with col_c2:
        num_buses = st.slider("🚍 Autobuses", 5, 40, 20, key="buses_cetram_a_unico")
    with col_c3:
        pasajeros_flota = st.slider("👥 Demanda Base [A] (Pax)", 500, 5000, 1800, step=100, key="p_flota_a_unico")
    with col_c4:
        horario_operativo = st.selectbox("🕒 Franja Horaria:", ["Pico Matutina", "Hora Valle", "Pico Nocturna"], key="horario_a_unico")
    ...
    # Contenedor principal estilo "Ambiente" responsivo
    st.markdown("""
        <div style="
...
with tab2:
    ...
    with col_d1:
        unidades_reparto = st.slider("🚚 Vehículos [T]", 1, 10, 3, key="slider_camiones_b_unico")
    with col_d2:
        pedidos_diarios = st.slider("💧 Pedidos Base [A] (Garrafones)", 50, 400, 150, step=10, key="slider_pedidos_b_unico")
    with col_d3:
        demanda_estacional = st.selectbox("🌤️ Temporada:", ["Regular", "Calor (Alta)"], key="d_estacional_b_unico")
